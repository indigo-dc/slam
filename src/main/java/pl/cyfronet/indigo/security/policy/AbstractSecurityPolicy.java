...
public abstract class AbstractSecurityPolicy<T> implements SecurityPolicy {
    ...
    static boolean hasAccessToActivity(Activity currentActivity,
            Collection<Activity> allowed) {
    ...
    static boolean hasRole(Identity identity, Role role) {
        SimpleGrantedAuthority testAuth = new SimpleGrantedAuthority("ROLE_"
                + role.toString());
